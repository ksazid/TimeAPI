        ...
        public dynamic EmployeeProductivityPerDateByEmpIDAndDate(string EmpID, string StartDate, string EndDate)
        {
            return Query<dynamic>(
                 sql: @"SELECT
                           ...
                           Isnull(Format(Cast(check_out AS DATETIME2), N'hh:mm tt'), '00:00') AS check_out,
                           Isnull(break_hours, '00:00') AS break_hours,
                           Isnull(total_hrs, '00:00') AS total_hrs,
                           Isnull(CONVERT(VARCHAR(5), Dateadd(minute,(Datediff(minute, Isnull( break_hours , '00:00'), total_hrs)), 0 ), 114), '00:00') AS final_total_hours,
                        ...
                        FROM
                           (
                              SELECT
                                 ...
                                 timesheettotalhrs.total_hrs AS total_hrs,
                              ...
                              FROM
                                 ...
                                 LEFT JOIN
                                    (
                                       ...
                                       WHERE
                                          Format(Cast(timesheet.ondate AS DATE), 'MM/dd/yyyy', 'EN-US') 
                                          BETWEEN Format( Cast(@StartDate AS DATE), 'MM/dd/yyyy', 'EN-US') 
                                          AND Format(Cast(@EndDate AS DATE), 'MM/dd/yyyy', 'EN-US')
                                 ...
                                 LEFT JOIN
                                    (
                                       SELECT
                                          timesheet.empid,
                                          Format(Cast(timesheet.ondate AS DATE), 'MM/dd/yyyy', 'EN-US') ondate,
                                          CONVERT(VARCHAR(5), Dateadd (minute, Sum(Datediff(minute, timesheet.check_in, timesheet.check_out)), 0), 114) AS total_hrs
                                       FROM
                                          timesheet
                                       WHERE
                                          Format(Cast(timesheet.ondate AS DATE), 'MM/dd/yyyy', 'EN-US') 
                                    ...
                                    AS timesheettotalhrs
                                    ON timesheetFirst.ondate = timesheettotalhrs.ondate
                              WHERE
        ...
        public dynamic DesktopEmployeeProductivityPerDateByEmpIDAndDate(string EmpID, string StartDate, string EndDate)
        {
            return Query<dynamic>(
                 sql: @"SELECT
                              ...
                              ISNULL(CONVERT(varchar(5), DATEADD(minute, 
                              (
                                DATEDIFF(minute, ISNULL(private_time, '00:00'), total_hrs)
                              )
                            , 0), 114), '00:00') AS final_total_hours,
                              FORMAT(CAST(ondate AS DATE), 'MM/dd/yyyy', 'EN-US') as ondate 
                            FROM
                              (
                                SELECT
                                ...
                                FROM
                                  ...
                                  LEFT JOIN
                                    (
                                      SELECT
                                        FORMAT(Cast(timesheet_desk.ondate AS DATE), 'MM/dd/yyyy', 'EN-US') ondate,
                                        CONVERT(VARCHAR(5), Dateadd (minute, Sum(Datediff(minute, timesheet_desk.check_in, timesheet_desk.check_out)), 0), 114) AS total_hrs 
                                      FROM
                                        timesheet_desk 
                                      WHERE
                                        FORMAT(Cast(timesheet_desk.ondate AS DATE), 'MM/dd/yyyy', 'EN-US') 
                                        BETWEEN FORMAT( Cast(@StartDate AS DATE), 'MM/dd/yyyy', 'EN-US') 
                                        AND FORMAT(Cast(@EndDate AS DATE), 'MM/dd/yyyy', 'EN-US') 
                                        AND timesheet_desk.empid = @EmpID 
                                      GROUP BY
                                        FORMAT(Cast(timesheet_desk.ondate AS DATE), 'MM/dd/yyyy' , 'EN-US') 
                                    )
                                    AS timesheettotalhrs 
                                  ...
                                  LEFT JOIN
                                    (
                                      SELECT 
										 ...
										 (timesheetproductivePercent.totalcount)* 100 / COUNT(xx.id) as ratio
                                    ...
                                    INNER JOIN
                                        (
											...
											COUNT(*) AS TOTALCOUNT
											  ...
											  WHERE
												...
												BETWEEN FORMAT(Cast('07/1/2020' AS DATE), 'MM/dd/yyyy', 'EN-US')
												AND FORMAT(Cast(@EndDate AS DATE), 'MM/dd/yyyy', 'EN-US') 
												AND emp_id = @EmpID  
												AND employee_app_tracked.app_name <> ' '
                                        ...
                                        )  as timesheetproductivePercent ON FORMAT(Cast( xx.created_date AS DATE), 'MM/dd/yyyy', 'EN-US') = FORMAT(Cast(  timesheetproductivePercent.ONDATE  AS DATE), 'MM/dd/yyyy', 'EN-US') 
									...
									where FORMAT(Cast(xx.created_date AS DATE), 'MM/dd/yyyy', 'EN-US')
                                          ...
                                          GROUP BY
                                            FORMAT(Cast(xx.created_date AS DATE), 'MM/dd/yyyy' , 'EN-US') ,
