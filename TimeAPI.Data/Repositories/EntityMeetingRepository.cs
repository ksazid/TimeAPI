...
namespace TimeAPI.Data.Repositories
{
    public class EntityMeetingRepository : RepositoryBase, IEntityMeetingRepository
    {
        ...
        public EntityMeeting Find(string key)
        {
            return QuerySingleOrDefault<EntityMeeting>(
                sql: "SELECT * FROM dbo.entity_meeting WHERE id = @key and is_deleted = 0",
                param: new { key }
            );
        }

        public IEnumerable<EntityMeeting> All()
        {
            return Query<EntityMeeting>(
                sql: "SELECT * FROM dbo.entity_meeting where is_deleted = 0"
            );
        }

        public IEnumerable<EntityMeeting> EntityMeetingByOrgID(string key)
        {
            return Query<EntityMeeting>(
        ...
        public dynamic EntityMeetingByEntityID(string key)
        {
        ...
        public dynamic GetAllOpenActivitiesEntityID(string key)
        {
            return Query<dynamic>(
                sql: @"SELECT 
                            dbo.task.id,
        ...
        public dynamic GetAllCloseActivitiesEntityID(string key)
        {
            return Query<dynamic>(
                sql: @"SELECT   *  FROM (
                                SELECT 
								...
								AND (dbo.task.is_local_activity = 0 or timesheet_activity.milestone_id is null)
